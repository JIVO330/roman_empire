...
### new tab to improve in version Anthony
      tabPanel(
        tags$i("Calculus"),
        tags$audio(src = "music/rome4.mp3",type ="audio/mp3",autoplay = TRUE, controls = NA),
        br(),
        h3(" The boring stuff"),
        h3("CITIES BY IMPERIAL PROVINCE/ COUNTRY ADN CIVIL STATUS"),
        fluidRow(
        column(3,
               selectInput("imperial_province2", label = h3(tags$i("Imperial Province")), #h3 size phrase()
                           choices = imperial_province)
        ),
        column(3,
               selectInput("country2", label = h3(tags$i("Modern Country")), #h3 size phrase()
                           choices = country)
        ),
        column(3,
               selectInput("status", label = h3(tags$i("Civic Status")), #h3 size phrase()
                           choices = status)
        ),
